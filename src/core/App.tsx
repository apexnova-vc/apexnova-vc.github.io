...
const App: React.FC = () => {
  ...
  useEffect(() => {
    axios
      .get<Item[]>(`${API_BASE_URL}/questions`)
  ...
  const handleSubmitNewItem = (event: FormEvent<HTMLFormElement>): void => {
    ...
    if (newItem.trim() !== "") {
      ...
      axios
        .post<Item>(`${API_BASE_URL}/question`, {
          title: "How do I log in to our password store?",
          content: "My desktop password did not work",
        })
        .then((response) => {
  ...
  const saveItem = (id: string): void => {
    if (editText.trim() !== "") {
      axios
        .put<Item>(`${API_BASE_URL}/data/${id}`, { data: editText })
        .then((response) => {
          setItems(
  ...
  return (
    <div style={styles.container}>
      ...
      <form onSubmit={handleSubmitNewItem} style={styles.form}>
        <label htmlFor="newItem" style={styles.label}>
          New Item:
        </label>
        <input
          id="newItem"
